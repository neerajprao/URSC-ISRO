...
def run_optimization():
    ...
    for idx, (component, is_back) in enumerate(components_to_process):
        ...
        if shape == 'rectangle':
            ...
            # 1. Handle 2 inserts (midpoints of shorter edges)
            if insert_qty == 2:
                if length >= width:
                    # Shorter edges are vertical (left and right)
                    offsets = [(-half_l, 0.0), (half_l, 0.0)]
                else:
                    # Shorter edges are horizontal (top and bottom)
                    offsets = [(0.0, -half_w), (0.0, half_w)]
                    
            # 2. Handle 4 or more even inserts
            else:
                # Place 4 corners first
                offsets = [
                    (half_l, half_w), 
                    (half_l, -half_w), 
                    (-half_l, half_w), 
                ...
                # Distribute remaining inserts evenly on the 2 longer sides
                remaining = insert_qty - 4
                if remaining > 0:
                    pins_per_side = remaining // 2
                    
                    if length >= width:
                        # Longer sides are horizontal (top and bottom)
                        x_steps = np.linspace(-half_l, half_l, pins_per_side + 2)[1:-1]
                        for x in x_steps:
                            offsets.append((x, half_w))   # Top edge
                            offsets.append((x, -half_w))  # Bottom edge
                    else:
                        # Longer sides are vertical (left and right)
                        y_steps = np.linspace(-half_w, half_w, pins_per_side + 2)[1:-1]
                        for y in y_steps:
                            offsets.append((half_l, y))   # Right edge
                            offsets.append((-half_l, y))  # Left edge
    ...
    if len(distinct_layouts) > 0:
        ...
        for idx, layout in enumerate(distinct_layouts):
            ...
            # Create high-resolution figure canvas (Increased height and expanded hspace to eliminate table overlap)
            fig_c = plt.figure(figsize=(16, 13))
            gs_c = fig_c.add_gridspec(2, 1, height_ratios=[6.5, 2.0], hspace=0.35)
            ...
            if close_pairs_labels:
                ...
                summary_box = ax_c.text(1.02, 0.95, f"Pin Distances Summary:\n\n{legend_box_text}", 
            ...
            # Render text box indicating full pin spacing compliance if no close pairs exist
                ...
                summary_box = ax_c.text(1.02, 0.95, "Pin Distances Summary:\n\nNo insert violations\nor pins found under 40mm.", 
                                        transform=ax_c.transAxes, fontsize=9, verticalalignment='top',
                                        bbox=dict(boxstyle="round,pad=0.5", fc="#F8F9F9", ec="#BDC3C7", lw=1.2))
            ...
            back_patch = mpatches.Patch(facecolor='none', edgecolor='darkgreen', linestyle='--', label='Back Layer Components\n(Projected Outline)')
            
            # Place legend box directly below the Pin Distances Summary box
            ax_c.legend(handles=[front_patch, back_patch], bbox_to_anchor=(1.02, 0.45), loc='upper left', fontsize=8.5)
